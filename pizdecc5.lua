...
-- ИСПРАВЛЕННОЕ соединение - НЕ блокирует HumanoidRootPart
local rootWeld = Instance.new("WeldConstraint")
rootWeld.Part0 = hrp
rootWeld.Part1 = model.PrimaryPart
rootWeld.Parent = model.PrimaryPart  -- ВАЖНО: parent в модели, а не в HRP!

-- Система привязки к костям персонажа для анимации (ИСПРАВЛЕННАЯ)
...
-- ИСПРАВЛЕННАЯ привязка - не трогаем HumanoidRootPart!
local function bindModelToCharacter(obj)
	for _, part in ipairs(obj:GetChildren()) do
		if part:IsA("BasePart") and part ~= model.PrimaryPart then
			...
			if targetBodyPart and targetBodyPart ~= "HumanoidRootPart" then
				local characterPart = character:FindFirstChild(targetBodyPart)
				if characterPart and characterPart ~= hrp then  -- НЕ привязываем к HRP!
				...
				-- Если нет соответствия, привязываем к главной части модели
				local weld = Instance.new("WeldConstraint")
